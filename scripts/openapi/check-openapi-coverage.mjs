...
function addComponents(spec) {
  ...
  schemas.AgentPublicAttribution = { type: 'object', required: ['clientName', 'runId'], properties: { clientName: { type: 'string' }, runId: { type: 'string' }, model: { type: 'string' }, provider: { type: 'string' }, purpose: { type: 'string' }, sources: { type: 'array', items: { type: 'object', properties: { url: { type: 'string', format: 'uri' }, artifactId: { type: 'string' }, checksum: { type: 'string' } } } } } };
  schemas.StructuredDebateContributionRequest = { type: 'object', required: ['contributionType', 'content', 'evidenceLinks'], properties: { contributionType: { type: 'string', enum: ['argument', 'evidence', 'response', 'clarification', 'closing'] }, content: { type: 'string', minLength: 40, maxLength: 2000 }, evidenceLinks: { type: 'array', minItems: 1, maxItems: 5, items: { type: 'object', required: ['url'], properties: { url: { type: 'string', format: 'uri' }, label: { type: 'string', maxLength: 160 } } } } } };
...
function specialize(spec) {
  ...
  if (paths['/structured-debates/{id}/contributions']?.post) Object.assign(paths['/structured-debates/{id}/contributions'].post, { summary: 'Submit a labelled contribution for an already-consented participant', description: 'Agent credentials may contribute only through their accountable user when that user already joined the pilot. Agent authorship is public and has no verdict impact.', requestBody: { required: true, content: { 'application/json': { schema: { $ref: '#/components/schemas/StructuredDebateContributionRequest' } } } } });
