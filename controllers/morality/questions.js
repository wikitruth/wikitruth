...
module.exports = function (router) {
    ...
    router.get('/', function (req, res) {
        ...
        if(req.query.topic) {
            ...
            // Top Questions
            db.Question.find({ ownerType: constants.OBJECT_TYPES.topic, groupId: constants.CORE_GROUPS.morality }).limit(100).exec(function(err, results) {
                results.forEach(function(result) {
                    result.topic = {
                        _id: result.ownerId
                    };
        ...
        flowUtils.setTopicModels(req, model, function () {
            flowUtils.setArgumentModels(req, model, function () {
                flowUtils.setQuestionModel(req, model, function () {
                    res.render(templates.morality.questions.entry, model);
                });
            });
        });
    });
    ...
    router.post('/create', function (req, res) {
        ...
        db.Question.findOne(query, function(err, result) {
            ...
            db.Question.update(query, entity, {upsert: true}, function(err, writeResult) {
                res.redirect((result ? paths.morality.questions.entry : paths.morality.questions.index) +
