        ...
        <div className="wt-admin-section-body flush">
          ...
          {data?.items.length ? <table className="wt-admin-table responsive">
            ...
            <tbody>{data.items.map((item) => <tr key={item.id}>
              ...
              <td data-label="Activity"><strong>{item.contributionCount}</strong> contribution{item.contributionCount === 1 ? '' : 's'}</td>
              <td data-label="Security"><span title="Active sessions"><i className="fa fa-laptop" aria-hidden="true" /> {item.activeSessions}</span>{' · '}<span title="Active passkeys"><i className="fa fa-key" aria-hidden="true" /> {item.activePasskeys}</span>{' · '}<span title="Active API clients"><i className="fa fa-code" aria-hidden="true" /> {item.activeApiClients}</span></td>
              ...
              <td data-label="Actions"><Link className="btn btn-default btn-xs" to={`/admin/users/${encodeURIComponent(item.id)}`}>Review</Link>{' '}<button className="btn btn-link btn-xs" type="button" onClick={() => void startPreview(item.state === 'quarantined' || item.state === 'deactivated' ? 'restore' : 'quarantine', [item.id])}>{item.state === 'active' ? 'Quarantine' : 'Restore'}</button></td>
            </tr>)}</tbody>
          </table> : null}
        </div>
