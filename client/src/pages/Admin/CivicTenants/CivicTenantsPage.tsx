...
type FormState = {
  ...
  locale: string; timezone: string; currency: string; primaryColor: string; accentColor: string; surfaceColor: string;
  levels: string; sections: string;
};

const EMPTY_FORM: FormState = {
  tenantId: '', title: '', navTitle: '', countryCode: '', domains: '', slogan: '', locale: 'en', timezone: 'UTC', currency: 'USD',
...
function formFromTenant(tenant: CivicTenant): FormState {
  return {
    tenantId: tenant.tenantId, title: tenant.title, navTitle: tenant.navTitle, countryCode: tenant.countryCode,
    domains: tenant.domains.join(', '), slogan: tenant.slogan, locale: tenant.localization.defaultLocale,
    ...
    levels: JSON.stringify(tenant.geography.levels, null, 2), sections: JSON.stringify(tenant.sections, null, 2),
  };
}
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload: CivicTenant = {
        tenantId: form.tenantId.trim().toLowerCase(), status: 'active', countryCode: form.countryCode.trim().toUpperCase(),
        ...
        branding: { logoIcon: '', favicon: '', primaryColor: form.primaryColor, accentColor: form.accentColor, surfaceColor: form.surfaceColor, fontFamily: '' },
        localization: { defaultLocale: form.locale, supportedLocales: [form.locale], timezone: form.timezone, currency: form.currency.toUpperCase() },
        geography: { levels, addressFields: levels.map((level) => level.key).filter((key) => key !== 'country') },
        sections, featureFlags: { observations: true, projects: true, elections: true, knowledgeLinks: true },
  ...
  return <div>
    ...
    <div className="row">
      ...
      <div className="col-md-7"><form className="panel panel-default" onSubmit={submit}><div className="panel-heading"><strong>{editing ? `Edit ${form.tenantId}` : 'Create tenant'}</strong></div><div className="panel-body">
        ...
        <div className="row"><div className="col-sm-6 form-group"><label htmlFor="tenant-locale">Default locale</label><input id="tenant-locale" className="form-control" required value={form.locale} onChange={(event) => setField('locale', event.target.value)} /></div><div className="col-sm-6 form-group"><label htmlFor="tenant-timezone">Timezone</label><input id="tenant-timezone" className="form-control" required value={form.timezone} onChange={(event) => setField('timezone', event.target.value)} /></div></div>
