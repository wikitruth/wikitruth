...
| Journal route parity | header “My Journal” modern route behavior (with legacy diary alias) | `/app/members/:username/journal` and `/app/members/profile/journal` (plus `/diary` aliases) | Done + Legacy retained | route config + manual smoke |
...
| Search parity | legacy tabbed + scoped search | modern `all/wiki/journal` scope (with legacy `diary` alias), tab routing, and “view more” behavior | Done + Legacy retained | `SearchPage.tsx`, `server/src/controllers/api/search.ts` |
| Contextual sidebar parity | legacy right-column contextual nav | modern contextual sidebar with section, related, journal/group shortcuts on md+ | Done + Legacy retained | `ContextSidebar.tsx`, layout integration |
...
| Parity-critical regression tests | route-level protections against parity regressions | dedicated parity checklist suite for auth, journal, entry actions, admin mutations, and social provider rendering (`npm run test:parity`, part of `test:ci`) | Done + Legacy retained | `tests/server/parity-checklist.test.js`, `package.json` |
