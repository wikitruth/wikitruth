...
- Commit `8c5f1910` removed the disabled `kraken-devtools` asset middleware from both development configs and refreshed `fast-uri` to `3.1.3`. Development startup, the full server suite, modern/legacy types, and the server build passed; audit is now `34` findings (`6` critical, `8` high, `17` moderate, `3` low), with every remaining high/critical finding in the retained template/localization chain.
- Commit `f12a049b` replaced vulnerable `autocannon` / `hyperid` with a Node 22 native concurrent load runner. All three live endpoint budgets passed (`17-23 ms` p95 and `731-810` average RPS); audit is now `32` findings (`6` critical, `8` high, `15` moderate, `3` low).
- Commit `32f06374` replaced unpatched `geopattern` / `extend` with an application-owned deterministic SVG mosaic generator. All `65` client suites / `156` tests, production build, smoke guardrails, PM2 restart, and visual Home-section verification passed; audit is now `30` findings (`6` critical, `8` high, `13` moderate, `3` low).
- Commit `4c20e578` upgraded Webpack Dev Server to `6.0.0` and raised the supported Node floor to `22.15.0`, which remains compatible with the Node 22/24 CI matrix and deployed Node `22.17.0` runtime. The development bundle, live dev-server response, and full smoke/type guardrails passed; audit is now `27` findings (`6` critical, `8` high, `10` moderate, `3` low).
- Commit `3d4ba87d` replaced the archived `csurf` package with an application-owned HMAC double-submit middleware. Existing modern headers, legacy `_csrf` form fields, signed-cookie behavior, monitoring exemptions, and `req.csrfToken()` remain compatible; tamper and cross-agent regression tests were added. All `47` server suites / `204` tests, smoke/type guardrails, and the server build passed; audit is now `25` findings (`6` critical, `8` high, `10` moderate, `1` low).
- Commit `2d39f1f4` refreshed both supported `js-yaml` branches to patched `3.15.0` / `4.3.0` releases. After reproducing one transient HTTP parser failure five times without recurrence, the full `47` server suites / `205` tests and `65` client suites / `156` tests passed; audit is now `24` findings (`6` critical, `8` high, `9` moderate, `1` low).
- Commit `f3b7185a` migrated the component workbench from Storybook 8 to ESM-only Storybook `10.5.0`, replacing the removed Essentials/Interactions packages with core features plus `@storybook/addon-docs`. Storybook Doctor, all `27` generated story variants, Browser-rendered controls, the client suite/build, and smoke/type guardrails passed.
- `npm audit` and `npm audit --omit=dev` are now both `21` findings (`6` critical, `8` high, `6` moderate, `1` low). Every remaining finding is isolated to the retained Jade/Dust/Kraken legacy comparison renderer; the exact boundary and exit condition are published in `docs/qa/DEPENDENCY_AUDIT_2026-07-12.md`.
