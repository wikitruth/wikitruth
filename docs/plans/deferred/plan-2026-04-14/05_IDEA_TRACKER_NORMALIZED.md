...
| CORE-002 | Core Platform | todo | Implement strict duplicate prevention and duplicate merge workflow for Topic, Argument, and Question. | Content Duplicates, Backlog, Research Features. |
...
| CORE-010 | Core Platform | todo | Implement onboarding requirements for Contributor/Reviewer progression. | Users, Note to self, Wikitruth Notes. |
...
| CORE-014 | Core Platform | todo | Implement Change Request (CR) workflow with partial accept/reject and diff preview. | Change Request (CR), Versioning & History. |
| CORE-015 | Core Platform | todo | Implement conflict handling for stale CRs and manual merge dispute flow. | Change Request (CR). |
...
| CORE-029 | Core Platform | todo | Implement internal artifact storage plus external-link mode. | Development Phases, Wikitruth Tasks. |
...
| CORE-034 | Core Platform | todo | Implement immutable audit timeline for all privileged actions. | Backlog, Mechanics & Architecture. |
...
| FLOW-010 | Product Workflows | todo | Implement discussion obsolescence tagging when parent argument revisions invalidate old comments. | Scenarios & Use Cases, Versioning & History. |
...
| CONTENT-001 | Content Ops | todo | Publish contributor/reviewer guidelines (allowed, discouraged, prohibited patterns). | Note to self, Backlog, Wikitruth Notes, Project Overview. |
| CONTENT-002 | Content Ops | todo | Publish issue taxonomy handbook with examples and moderation outcomes. | Issues, Backlog, Wikitruth Tasks. |
| CONTENT-003 | Content Ops | todo | Publish verdict policy handbook distinguishing Verified, Pending, Likely, Unsupported, False, Partial. | Acceptance & Verdict, Backlog, Wikitruth Notes. |
| CONTENT-004 | Content Ops | todo | Publish duplicate/similarity policy and merge decision rules. | Content Duplicates, Backlog, Wikitruth Tasks. |
| CONTENT-005 | Content Ops | todo | Publish truth-vs-ethics separation policy with examples. | Wikitruth Notes, Backlog, Mechanics & Architecture. |
| CONTENT-006 | Content Ops | todo | Define source quality rubric for artifacts and references. | Mechanics & Architecture, References & Related Materials, Note to self. |
...
| CONTENT-011 | Content Ops | todo | Add “fragile fact” date labeling policy for rapidly changing claims. | Backlog, Research Features. |
| CONTENT-012 | Content Ops | todo | Create examples library for each entry type and quality level. | Backlog, Wikitruth Notes. |
| CONTENT-013 | Content Ops | todo | Stand up screening operations playbook (pending, approved, rejected, returned with issues). | Backlog, Wikitruth Tasks. |
| CONTENT-014 | Content Ops | todo | Stand up reviewer operations playbook (verdict assignment, issue escalation, appeal handling). | Development Phases, Backlog, Issues. |
| CONTENT-015 | Content Ops | todo | Implement stale discussion cleanup procedures (obsolescence tags, archive, takeover windows). | Scenarios & Use Cases, Mechanics & Architecture. |
| CONTENT-016 | Content Ops | todo | Define and enforce concise-writing standard to prevent encyclopedic drift. | Note to self, Mechanics & Architecture, Wikitruth Notes. |
| CONTENT-017 | Content Ops | todo | Define contributor incentives (credits, attribution, profile progress) tied to quality. | Scenarios & Use Cases, Backlog, Wikitruth Tasks. |
| FIXPH-001 | FixPH | todo | Implement FixPH section architecture (Issues & Events, People, Groups, Projects, Actions, Vote Wisely, History). | fixthephilippines.org/fixthephilippines.org, FixthePH Notes. |
| FIXPH-002 | FixPH | todo | Implement government/entity graph model (country -> institution -> office -> person + incidents/issues/projects). | fixthephilippines.org/fixthephilippines.org, fixtheph.smmx, Linking.smmx. |
| FIXPH-003 | FixPH | todo | Implement accountability views for projects (budget, officials, contract, timeline, progress evidence). | fixthephilippines.org/fixthephilippines.org, FixthePH Notes. |
| FIXPH-004 | FixPH | todo | Implement citizen observation uploads and issue escalation flow. | fixthephilippines.org/fixthephilippines.org, FixthePH Notes. |
| FIXPH-005 | FixPH | todo | Implement location-aware issue/event surfacing for citizens. | FixthePH Notes, Development Phases. |
| FIXPH-006 | FixPH | todo | Implement election guidance workspace with candidate issue profiles. | FixthePH Notes, Topics to create. |
| FIXPH-007 | FixPH | todo | Implement issue severity and stage model for public incident tracking. | FixthePH Notes, Wikitruth Tasks. |
| FIXPH-008 | FixPH | todo | Implement long-lived historical memory mechanism (“system does not forget”). | FixthePH Notes, fixthephilippines.org/fixthephilippines.org. |
| CONTENT-018 | Content Ops | todo | Run controlled pilot with 2-person custom debate on seeded topic. | Development Phases. |
